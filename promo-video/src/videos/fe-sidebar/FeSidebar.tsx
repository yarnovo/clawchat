...
import fesIntroWords from "./words/fes-intro-words.json";
import fesSidebarWords from "./words/fes-sidebar-words.json";
...
const SCENE_COMPS = [SceneIntro, SceneSidebar];
const SCENE_WORDS = [fesIntroWords, fesSidebarWords];
...
export const FE_SIDEBAR_FRAMES = scenes[scenes.length - 1].from + scenes[scenes.length - 1].dur;
...
export const FeSidebar: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: "#FAF9F6" }}>
      <Audio src={staticFile("audio/fe-sidebar/fe-sidebar.mp3")} volume={0.9} />
      {scenes.map((s, i) => {
