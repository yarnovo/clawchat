...
import { SceneSse } from "./SceneSse";
import { SceneFlow } from "./SceneFlow";
import { Subtitle } from "../../Subtitle";

import introWords from "./words/svp-intro-words.json";
import sseWords from "./words/svp-sse-words.json";
import flowWords from "./words/svp-flow-words.json";
...
const SCENE_COMPS = [SceneIntro, SceneSse, SceneFlow];
const SCENE_WORDS = [introWords, sseWords, flowWords];
...
export const SV_PROXY_FRAMES = scenes[scenes.length - 1].from + scenes[scenes.length - 1].dur;
...
export const SvProxy: React.FC = () => (
  <AbsoluteFill style={{ backgroundColor: "#FAF9F6" }}>
    <Audio src={staticFile("audio/sv-proxy/sv-proxy.mp3")} volume={0.9} />
    {scenes.map((s, i) => {
