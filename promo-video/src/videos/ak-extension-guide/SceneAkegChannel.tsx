...
const codeLines = [
  { text: "// 实现 Channel 接口", color: COLORS.muted },
  { text: "function wsChannel(opts): Channel {", color: COLORS.text },
  { text: "  return {", color: COLORS.text },
  { text: "    name: 'websocket',", color: COLORS.accent },
  { text: "    setup: async (ctx) => {", color: COLORS.text },
  { text: "      // ctx.eventLoop.push(event)", color: COLORS.muted },
...
export const SceneAkegChannel: React.FC = () => {
    ...
    <AbsoluteFill>
      ...
      <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", flexDirection: "column", gap: 32, paddingBottom: 140 }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 16 }}>
          <div style={{
            fontFamily: FONT, fontSize: 56, fontWeight: 700, color: COLORS.muted,
            opacity: interpolate(titleProg, [0, 1], [0, 1]),
          }}>Channel</div>
          <div style={{
            fontFamily: FONT_SANS, fontSize: 28, color: COLORS.muted,
            opacity: interpolate(titleProg, [0, 1], [0, 1]),
          }}>— setup 拿 EventLoop，push 事件进去</div>
        </div>
