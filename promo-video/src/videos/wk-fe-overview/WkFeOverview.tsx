...
import { ScenePkg } from "./ScenePkg";
import { SceneVite } from "./SceneVite";
import { SceneEntry } from "./SceneEntry";
import { Subtitle } from "../../Subtitle";

import wkfeoPkgWords from "./words/wkfeo-pkg-words.json";
import wkfeoViteWords from "./words/wkfeo-vite-words.json";
import wkfeoEntryWords from "./words/wkfeo-entry-words.json";
...
const SCENE_COMPS = [ScenePkg, SceneVite, SceneEntry];
const SCENE_WORDS = [wkfeoPkgWords, wkfeoViteWords, wkfeoEntryWords];
...
export const WK_FE_OVERVIEW_FRAMES = scenes[scenes.length - 1].from + scenes[scenes.length - 1].dur;
...
export const WkFeOverview: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: "#FAF9F6" }}>
      <Audio src={staticFile("audio/wk-fe-overview/wk-fe-overview.mp3")} volume={0.9} />
      {scenes.map((s, i) => {
