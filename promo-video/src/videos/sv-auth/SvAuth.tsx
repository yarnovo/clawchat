...
import { SceneSvauIntro } from "./SceneSvauIntro";
import { SceneSvauFlow } from "./SceneSvauFlow";
import { Subtitle } from "../../Subtitle";

import svauIntroWords from "./words/svau-intro-words.json";
import svauFlowWords from "./words/svau-flow-words.json";
...
const SCENE_COMPS = [SceneSvauIntro, SceneSvauFlow];
const SCENE_WORDS = [svauIntroWords, svauFlowWords];
...
export const SV_AUTH_FRAMES = scenes[scenes.length - 1].from + scenes[scenes.length - 1].dur;
...
export const SvAuth: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: "#FAF9F6" }}>
      <Audio src={staticFile("audio/sv-auth/sv-auth.mp3")} volume={0.9} />
      {scenes.map((s, i) => {
