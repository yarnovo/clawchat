...
import femIntroWords from "./words/fem-intro-words.json";
import femCustomWords from "./words/fem-custom-words.json";
...
const SCENE_COMPS = [SceneIntro, SceneCustom];
const SCENE_WORDS = [femIntroWords, femCustomWords];
...
export const FE_MARKDOWN_FRAMES = scenes[scenes.length - 1].from + scenes[scenes.length - 1].dur;
...
export const FeMarkdown: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: "#FAF9F6" }}>
      <Audio src={staticFile("audio/fe-markdown/fe-markdown.mp3")} volume={0.9} />
      {scenes.map((s, i) => {
