...
import festIntroWords from "./words/fest-intro-words.json";
import festChatWords from "./words/fest-chat-words.json";
...
const SCENE_COMPS = [SceneIntro, SceneChat];
const SCENE_WORDS = [festIntroWords, festChatWords];
...
export const FE_STORES_FRAMES = scenes[scenes.length - 1].from + scenes[scenes.length - 1].dur;
...
export const FeStores: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: "#FAF9F6" }}>
      <Audio src={staticFile("audio/fe-stores/fe-stores.mp3")} volume={0.9} />
      {scenes.map((s, i) => {
