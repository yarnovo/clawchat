...
import { SceneChatStore } from "./SceneChatStore";
import { SceneUiStore } from "./SceneUiStore";
import { Subtitle } from "../../Subtitle";

import wkfestChatWords from "./words/wkfest-chat-words.json";
import wkfestUiWords from "./words/wkfest-ui-words.json";
...
const SCENE_COMPS = [SceneChatStore, SceneUiStore];
const SCENE_WORDS = [wkfestChatWords, wkfestUiWords];
...
export const WK_FE_STORES_FRAMES = scenes[scenes.length - 1].from + scenes[scenes.length - 1].dur;
...
export const WkFeStores: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: "#FAF9F6" }}>
      <Audio src={staticFile("audio/wk-fe-stores/wk-fe-stores.mp3")} volume={0.9} />
      {scenes.map((s, i) => {
