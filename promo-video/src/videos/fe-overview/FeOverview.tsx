...
import feoIntroWords from "./words/feo-intro-words.json";
import feoStackWords from "./words/feo-stack-words.json";
import feoStructureWords from "./words/feo-structure-words.json";
...
const SCENE_COMPS = [SceneIntro, SceneStack, SceneStructure];
const SCENE_WORDS = [feoIntroWords, feoStackWords, feoStructureWords];
...
export const FE_OVERVIEW_FRAMES = scenes[scenes.length - 1].from + scenes[scenes.length - 1].dur;
...
export const FeOverview: React.FC = () => {
  return (
    <AbsoluteFill style={{ backgroundColor: "#FAF9F6" }}>
      <Audio src={staticFile("audio/fe-overview/fe-overview.mp3")} volume={0.9} />
      {scenes.map((s, i) => {
