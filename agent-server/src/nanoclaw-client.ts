// HTTP client for nanoclaw-server — manage NanoClaw instances

const NANOCLAW_SERVER_URL =
  process.env["NANOCLAW_SERVER_URL"] || "http://localhost:3005";
...
async function request(path: string, init?: RequestInit, requestId?: string) {
  ...
  const res = await fetch(
    `${NANOCLAW_SERVER_URL}/v1/nanoclaw${path}`,
    { ...init, headers },
  );
...
export async function createInstance(opts: {
  ...
  if (res.status !== 201) {
    throw new Error(res.data.error || "Failed to create NanoClaw instance");
  }
  return res.data;
}
...
export async function startInstance(agentId: string, requestId?: string): Promise<void> {
  ...
  if (res.status !== 200) {
    throw new Error(res.data.error || "Failed to start NanoClaw instance");
  }
}
...
export async function stopInstance(agentId: string, requestId?: string): Promise<void> {
  ...
  if (res.status !== 200) {
    throw new Error(res.data.error || "Failed to stop NanoClaw instance");
  }
}
...
export async function removeInstance(
  ...
  if (res.status !== 200) {
    throw new Error(res.data.error || "Failed to remove NanoClaw instance");
  }
}
...
export async function chat(opts: {
  ...
  if (res.status !== 202 && res.status !== 200) {
    throw new Error(res.data.error || "NanoClaw chat failed");
  }
}
