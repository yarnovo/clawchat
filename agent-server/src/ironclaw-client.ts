// HTTP client for ironclaw-server — manage IronClaw instances

const IRONCLAW_SERVER_URL =
  process.env["IRONCLAW_SERVER_URL"] || "http://localhost:3006";
...
async function request(path: string, init?: RequestInit, requestId?: string) {
  ...
  const res = await fetch(
    `${IRONCLAW_SERVER_URL}/v1/ironclaw${path}`,
    { ...init, headers },
  );
...
export async function createInstance(opts: {
  ...
  if (res.status !== 201) {
    throw new Error(res.data.error || "Failed to create IronClaw instance");
  }
  return res.data;
}
...
export async function startInstance(agentId: string, requestId?: string): Promise<void> {
  ...
  if (res.status !== 200) {
    throw new Error(res.data.error || "Failed to start IronClaw instance");
  }
}
...
export async function stopInstance(agentId: string, requestId?: string): Promise<void> {
  ...
  if (res.status !== 200) {
    throw new Error(res.data.error || "Failed to stop IronClaw instance");
  }
}
...
export async function removeInstance(
  ...
  if (res.status !== 200) {
    throw new Error(res.data.error || "Failed to remove IronClaw instance");
  }
}
...
export async function chat(opts: {
  ...
  if (res.status !== 202 && res.status !== 200) {
    throw new Error(res.data.error || "IronClaw chat failed");
  }
}
