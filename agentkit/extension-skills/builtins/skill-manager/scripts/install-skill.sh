#!/usr/bin/env bash
# Install a skill from the marketplace to this agent's workspace.
...
RESPONSE=$(curl -sf -X POST \
  ...
  -d "{\"agentId\": \"${AGENT_ID}\"}" \
  "${SERVER_URL}/api/skills/${SKILL_NAME}/install" 2>&1) || {
  echo "Failed to install skill '${SKILL_NAME}'." >&2
  exit 1
}

echo "Skill '${SKILL_NAME}' installed successfully."
echo "It will be available in the next conversation turn."
