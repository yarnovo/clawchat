...
- **dry-run 必须尽可能模拟真实**：计算利润时要扣除手续费（0.1%）、考虑滑点、遵守最小下单量（$5 NOTIONAL），不能用理想化的假数据
- **KPI 以实盘真实利润为准**：dry-run 利润只作参考，真正的 KPI 是交易所账户的实际盈亏（总资产 - 初始投入）
- **报告必须包含真实数据**：所有发给用户的报告，资金部分必须从交易所拉真实数据，不能只用本地状态文件的模拟数据
